...
# ╔═╡ a7e24b20-77a6-11ec-0de3-937bb19deddb
using Flux, MLDatasets, Statistics, PlutoUI, MLUtils, Images, Plots

# ╔═╡ 90003345-4904-44e7-91bf-6eea1e5f6c5e
using Pipe: @pipe

# ╔═╡ 20aeef5e-fd90-4b15-b0b7-c3a7440d8384
using Metrics

# ╔═╡ 71939ad7-cf56-4267-9d59-22b13e84c1de
md"""
# Load The Data

A number of popular datasets are provided by the [MLDatasets](https://juliaml.github.io/MLDatasets.jl/stable/) package, which we use here to load both the training and test samples for the MNIST dataset.
"""

# ╔═╡ 6f28cde4-9bbf-4d14-872d-b04ef19d3090
begin
	train_x, train_y = MNIST(split=:train)[:]
	test_x, test_y = MNIST(split=:test)[:]
end;

# ╔═╡ 8b66b058-c9a5-4cd3-a6e9-1badc67c34a9
md"""
# Data Visualization

The first step in many machine learning tasks is to visualize our dataset. Fortunately, Julia provides the excellent [Plots](https://docs.juliaplots.org/stable/) library for just this occasion. Here we randomly select 10 samples from our training data and plot them in a 2x5 layout. For each sample, we provide its correct label as the title.
"""

# ╔═╡ 87b0ac82-ef6d-43c3-b715-42b737860b5e
function plot_sample(sample, label)
	@pipe sample |> 
	permutedims(_, (2, 1)) |> 
	Gray.(_) |> 
	plot(_, axis=nothing, showaxis=false, title=label, margin=0Plots.mm)
end;

# ╔═╡ b4105f7c-44f1-4d20-8590-36d2b65ef358
function visualize_data()
	samples = rand(1:size(train_y)[1], 10)
	plots = [plot_sample(train_x[:,:,i], train_y[i]) for i in samples]
	plot(plots..., axis=nothing, showaxis=false, layout=(2, 5), size=(1000, 450))
end;

# ╔═╡ bf152d43-d369-41f8-b8bf-ffdb7e18cba2
visualize_data()

# ╔═╡ b9079ef0-cf5a-404a-a72b-7aa643ddf88c
md"""
# Data Preparation

To make our labels compatible with Flux's `crossentropy` loss, we need to encode them as one-hot vectors. Fortunately, Flux provide the convenience function `onehotbatch(xs, labels)` for just this purpose. We also need to reshape our input features to be compatible with the Flux `Conv` layer, which expects a tensor with the shape (Width x Height x Channels x Observations). Finally, we pass our data into the `DataLoader` constructor, specifying a batch size of 32 and whether or not we want to shuffle our data between epochs.
"""

# ╔═╡ e6bc5027-ba21-4312-ac13-fba33882ad66
begin
	# Encode Labels As One-Hot Vectors
	train_labels = Flux.onehotbatch(train_y, 0:9) .|> float
	test_labels = Flux.onehotbatch(test_y, 0:9) .|> float

	# Add Channel Dimension To Features
	Xtrain = reshape(train_x, (28, 28, 1, 60000))
	Xtest = reshape(test_x, (28, 28, 1, 10000))

	# Prepare DataLoader
	data_train = DataLoader((Xtrain, train_labels), batchsize=32, shuffle=true)
	data_test = DataLoader((Xtest, test_labels), batchsize=32, shuffle=false)
end;

# ╔═╡ f224ba72-01fd-4d3c-bd3a-e70db66922ca
md"""
# Define The Model

Those of you who are familiar with Python's Keras library will likely feel right at home with Flux. The `Chain` constructor is more-or-less analogous to the keras `Sequential` model, where data is passed through each layer in sequence from first to last. Here we alternate between 3x3 convolutions and 2x2 max-pooling layers before flattening the tensor and passing it through two fully-connected layers and a final softmax activation. We also specify a ReLU activation on both convolutional and dense layers.
"""

# ╔═╡ 768a54e5-c3f0-4865-92c0-2efd663a33bb
model = Chain(
	Flux.Conv((3, 3), 1 => 32, relu, pad=SamePad()), 
	...
	Flux.MaxPool((2, 2), pad=SamePad()), 
	Flux.flatten, 
	Flux.Dense(2048, 100, relu),
	Flux.Dense(100, 10),
	Flux.softmax
);

# ╔═╡ 7a8d8c03-6ae8-471c-ac91-2b2693785d37
md"""
# Training

To train our model, Flux expects us to provide four things: 

1. A loss function.
2. The parameters we want to train. 
3. An optimizer.
4. Some training data.

Once we have these, we can use `Flux.train!(loss, pars, data, opt)` to train our model for a single epoch. To train for multiple epochs, we can use the macro `Flux.Optimize.@epochs`. However, the MNIST dataset is simple enough that we only require a single epoch. In fact, we could have left our call to `@epochs` out entirely, but it is shown here for demonstration purposes.
"""

# ╔═╡ 700827ce-a6ad-47b5-aa50-6fb30bdb9910
# ╠═╡ show_logs = false
begin
	# Define The Loss Function
	loss(x, y) = @pipe x |> model |> Flux.crossentropy(_, y)
	...
	# Train For One Epoch
	Flux.Optimise.@epochs 1 Flux.train!( loss, params, data_train, opt)
end

# ╔═╡ 3fe15f03-5d5b-4cfd-aa30-229e38025993
function evaluate_model(model, data)
	@pipe [(model(x), y) for (x, y) in data] |>
	[Metrics.categorical_accuracy(p...) for p in _] |> 
	round(mean(_) * 100, digits=2, base=10)
end;

# ╔═╡ ce2d8e3f-934e-4731-9164-2b9d9563ec32
md"""
# Evaluation

Now that we've trained our model, it's time to estimate its real-world performance by evaluating on some previously unseen data. Because each digit is equally represented in our dataset, we can use accuracy as a reasonable evaluation metric. If this was not the case, we may wish to employ alternative metrics such as precision, recall, and F-score.

#### Test Accuracy: $(evaluate_model(model, data_test))%
"""
...
git-tree-sha1 = "30c2390fdb76f6788540b83155bb63802de1814e"
...
git-tree-sha1 = "5ea5d81dddba5a0a04ef746b2c63a58ae56bf482"
